...
public class HangtechnikaToElimexQuery implements GlobalVariables {
	...
	public void stockType() {
		...
		//
		ELIMEX_HT.put("Megszünt, már nem tudjuk szállítani!", "Már nem szállítjuk");
	}
	...
	public void query() throws MalformedURLException, IOException {
		...
		indexOfStátusz = HANGZAVAR_MAP.get("columns").get("Cikkszám")
				.indexOf("Státusz (engedélyezett (1) v. letiltott (0) v. kifutott (2))");

		LinkedHashMap<String, ArrayList<String>> export = new LinkedHashMap<>();
		ELIMEX_UPLOAD_TO_SHOPRENTER.put("export", export);
		ELIMEX_UPLOAD_TO_SHOPRENTER.get("export").put("Cikkszám",
...
//            }
	...
	public void search(String k, String page) {
		...
		// String match_d_archivTermek = "";
		// String integerNettoAlaparString = "";
		Integer integerNettoAlapar = 0;

		Pattern pattern_a_nincsKeszletenAllapot = Pattern.compile("raktáron" + "|201\\d.\\d{2}.\\d{2}"
				+ "|a megrendeléstől számított 1-5 munkanap" + "|a megrendeléstől számított 5-10 munkanap"
				+ "|a megrendeléstől számított 10-15 munkanap" + "|a megrendeléstől számított 3-4 hét"
				+ "|a megrendeléstől számított 4-5 hét" + "|-- átmeneti készlethiány --" + "|Keressen a részletekért"
				+ "|csak előzetes ajánlat alapján" + "|rendeléskor egyeztetjük"
				+ "|Kifutó típus, már csak a készlet erejéig szállítható!" + "|Megszűnt! Már nem tudjuk szállítani!"
				+ "|Megszűnt! Már nem tudjuk szállítani!" + "|Megszünt, már nem tudjuk szállítani!");
		...
		Pattern pattern_d_archivTermek = Pattern.compile("ik_archiv3.jpg");
		...
		if (bodyText.contains(k)) {
			...
			Matcher matcher_d_archivTermek = pattern_d_archivTermek.matcher(doc.toString());
			if (matcher_d_archivTermek.find()) {
				match_a_nincsKeszletenAllapot = "Megszűnt! Már nem tudjuk szállítani!";// fake
			}
			...
			if (matcher_c_bruttoAlapar.find()) {
				...
				// Integer integerNettoAlapar = (int)
				// ((Float.parseFloat(match_c_bruttoAlapar.replace(" ", "")) + 0.5) / 1.27);
